...
string next_page = "";
string matrice = "";
string nome_file_excel = "Test_2.csv";
...
void Conta_elementi()
{
	fstream fin;
	int cont = 0;
	fin.open(nome_file_excel.c_str());
	if(fin.bad())
...
void Rettangolo(int Height,int Width,int Top, int Left )
{
...
void IO_field(int Height,int Top, string PLC_tag, int Left,int Width,bool Input)
{
	string Mode = "Input";
	if(!Input)
	string Mode = "Output";
	...
	string Text_name = "Testo_";
	ofstream fin;
	fin.open(nome_pagina_hmi.c_str(),ios::out|ios::app);	
	fin<<
...
"              <Flashing>None</Flashing>\n"
"              <ForeColor>49, 52, 74</ForeColor>\n"
"              <FormatPattern>s99999</FormatPattern>\n"
"              <Height>"<<Height<<"</Height>\n"
...
void Parametriche()
{
	...
	for(int i = 0;i<=n_pagine;i++)
	{
		...
		for(int j = 0;j<Pagine[i].n_riquadri;j++)
		{
			Inizio_Gruppo();
			// Rettangolo
			Rettangolo(Pagine[i].riquadro[j].Height,Pagine[i].riquadro[j].Width,Pagine[i].riquadro[j].Top,Pagine[i].riquadro[j].Left);
			//Titolo
			int Titolo_Width = Pagine[i].riquadro[j].Tag.size()*14;
			int Titolo_Left = Pagine[i].riquadro[j].Left+10;
			int Titolo_Top = Pagine[i].riquadro[j].Top+5;
			int Titolo_Height = Y_t;
			
			Testo(Pagine[i].riquadro[j].Tag,Titolo_Height,Titolo_Width,Titolo_Top,Titolo_Left);
			...
			for( int k = 0;k<Pagine[i].riquadro[j].lunghezza;k++) // for every box
			{
				...
				//IO_field
				IO_field(30,Next_IO_T,Pagine[i].riquadro[j].nomi[k],Next_IO_L,70,0);
				...
				//IO_field
				IO_field(30,Next_IO_T,Pagine[i].riquadro[j].nomi[k],Next_IO_L,70,1);
...
int main(){
	...
	cout<<"Il file csv deve avere le seguenti colonne : TIPO_DATI,INDIRIZZI,TAG,PAROLA_CHIAVE,NOMI_COMPLETI,CHIUSURA\nLa colonna di CHIUSURA e'composta da soli ';'\n ";
	Start();
	Conta_elementi();
